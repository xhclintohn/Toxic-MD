import ownerMiddleware from '../../utils/botUtil/Ownermiddleware.js';
  ...
  async function gitUpdate(client, m, opts, root) {
      ...
      let branch = 'main';
      try { const { stdout } = await execAsync('git rev-parse --abbrev-ref HEAD', opts); if (stdout?.trim() && stdout.trim() !== 'HEAD') branch = stdout.trim(); } catch {}
      ...
      await sendInteractive(client, m, fmt('Updated → ' + head.trim() + '\n│ ' + depNote + '\n│ Restarting...'));
      doRestart(root);
  }

  async function herokuUpdate(client, m, apiKey, appName) {
      await sendInteractive(client, m, fmt('Heroku — triggering rebuild on ' + appName + '...'));
      await axios.post(`https://api.heroku.com/apps/${appName}/builds`, { source_blob: { url: `https://github.com/${REPO}/tarball/main` } }, { headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/vnd.heroku+json; version=3', 'Content-Type': 'application/json' }, timeout: 60000 });
      client.sendMessage(m.chat, { react: { text: '✅', key: m.reactKey } }).catch(() => {});
      return sendInteractive(client, m, fmt('Rebuild triggered for ' + appName + '.\n│ Bot restarts after build.'));
  }

  async function zipUpdate(client, m, opts, root) {
      let AdmZip; try { AdmZip = (await import('adm-zip')).default; } catch { return sendInteractive(client, m, fmt('adm-zip missing. Run npm install.')); }
      ...
      const res = await axios.get(`https://github.com/${REPO}/archive/refs/heads/main.zip`, { responseType: 'arraybuffer', timeout: 120000, maxContentLength: Infinity, maxBodyLength: Infinity, headers: { 'User-Agent': 'Toxic-MD' } });
      ...
      if (!fs.existsSync(srcDir)) { const dirs = fs.readdirSync(exRoot).map(d=>path.join(exRoot,d)).filter(p=>fs.statSync(p).isDirectory()); if (dirs.length) srcDir = dirs[0]; }
      fs.cpSync(srcDir, root, { recursive: true, force: true, filter: (src) => { const rel = path.relative(srcDir, src); if (!rel) return true; const top = rel.split(path.sep)[0]; if (SKIP.has(top)) return false; if (rel.endsWith('.session')) return false; if (top === 'database' && rel.endsWith('.db')) return false; return true; } });
      fs.rmSync(zipPath, { force: true }); fs.rmSync(exRoot, { recursive: true, force: true });
      let depNote = 'deps ok'; try { await execAsync('npm install --no-audit --no-fund', opts); depNote = 'deps installed'; } catch (e) { depNote = String(e.message||e).split('\n')[0]; }
      client.sendMessage(m.chat, { react: { text: '✅', key: m.reactKey } }).catch(() => {});
  ...
  export default {
      ...
      run: async (context) => {
          await ownerMiddleware(context, async () => {
              ...
              try {
                  if (isHeroku) {
                      const apiKey = getHerokuApiKey(); const appName = herokuAppName;
                      if (!apiKey || !appName) { client.sendMessage(m.chat, { react: { text: '❌', key: m.reactKey } }).catch(() => {}); return sendInteractive(client, m, fmt('Set HEROKU_API_KEY and HEROKU_APP_NAME first.')); }
                      return await herokuUpdate(client, m, apiKey, appName);
                  }
