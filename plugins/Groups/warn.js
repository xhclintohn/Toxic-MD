...
│ Admin only.\n╰───────────────\n> ©𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 𝐱𝐡_𝐜𝐥𝐢𝐧𝐭𝐨𝐧`);
        ...
        const picked = await resolveUserTarget(client, { ...context, participants: groupParticipants }, { participants: groupParticipants, chat: m.chat, order: ['quoted', 'mention', 'text'] });
        let rawJid = picked?.jid || m.quoted?.sender || m.mentionedJid?.[0];
        if (!rawJid) {
            await client.sendMessage(m.chat, { react: { text: '❌', key: m.reactKey } }).catch(() => {});
            return sendInteractive(client, m, `╭─❏ 「 WARN 」
