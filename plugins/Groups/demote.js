...
export default {
    ...
    run: async (context) => {
        await middleware(context, async () => {
            ...
            const picked = await resolveUserTarget(client, { ...context, participants }, { participants, chat: m.chat, order: ['quoted', 'mention', 'text'] });
            let rawJid = picked?.jid || null;
            if (!rawJid && m.quoted?.sender) {
                rawJid = m.quoted.sender;
            } else if (!rawJid && m.mentionedJid && m.mentionedJid.length > 0) {
...
│ Mention or quote a user. ${prefix}demote @user\n╰───────────────\n> ©𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 𝐱𝐡_𝐜𝐥𝐢𝐧𝐭𝐨𝐧`);
            ...
            const target = picked?.jid || resolveTargetJid(rawJid, participants) || pickActionableJid(rawJid, participants);
            if (!target) {
                await client.sendMessage(m.chat, { react: { text: '❌', key: m.reactKey } }).catch(() => {});
                return sendInteractive(client, m, `╭─❏ 「 DEMOTE 」
...
│ That command cannot be used on the dev or the bot.\n╰───────────────\n> ©𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 𝐱𝐡_𝐜𝐥𝐢𝐧𝐭𝐨𝐧`);
            ...
            try {
                ...
                await client.sendMessage(m.chat, {
                    text: `╭─❏ 「 DEMOTED」
│ ${picked?.label || '@' + target.split('@')[0]} got stripped of admin.\n│ Back to being a nobody.\n╰───────────────\n> ©𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 𝐱𝐡_𝐜𝐥𝐢𝐧𝐭𝐨𝐧`,
                    mentions: [target]
                });
            } catch (error) {
                await client.sendMessage(m.chat, { react: { text: '❌', key: m.reactKey } }).catch(() => {});
