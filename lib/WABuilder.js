...
/**
 * Sends a self-contained HTML/CSS/JS mini-app inline inside a chat bubble, using
 * the same internal `richResponseMessage` primitive Meta AI uses for its own
 * generative-UI answers inside WhatsApp.
 *
 * Read this before relying on it:
 * - Undocumented and unstable. The type is literally named
 *   `FOAHtmlPrimitiveDemoDONOTUSE` in Meta's own protocol — an internal/demo
 *   flag, not a published API. It can change or be pulled with any WhatsApp
 *   client update, with no notice and no fallback from Baileys.
 * - Sandboxed, one-way, client-side only. `trusted_sources: []` means the
 *   WebView can't fetch external resources, and there's no bridge back into
 *   the chat — the HTML cannot message the bot or update server-side state.
 *   Anything interactive has to be entirely self-contained in the HTML/JS
 *   (game logic, "AI" opponent, scoring — all run and settle locally).
 * - Size-limited. Payloads above roughly 2-3MB tend to silently fail to
 *   render (no error, the message just never appears). Keep it lean: no
 *   embedded images/fonts/large libraries.
 *
 * @param {object} sock - the Baileys socket
 * @param {string} jid - chat to send to
 * @param {string} html - full self-contained <html>...</html> document
 * @param {object} [opts]
 * @param {object} [opts.quoted] - best-effort reply context (support for this
 *   message type is unverified; omit if it doesn't render as expected)
 */
async function sendHtmlView(sock, jid, html, { quoted } = {}) {
  const payload = html.trim();
  const approxBytes = Buffer.byteLength(payload, "utf8");
  if (approxBytes > 1_500_000) {
    console.warn(
      `[sendHtmlView] payload is ~${(approxBytes / 1024 / 1024).toFixed(2)}MB — ` +
      `WhatsApp tends to silently drop these past ~2-3MB. Trim assets/scripts.`,
    );
  }

  const contextInfo = { isForwarded: true, forwardOrigin: 4 };
...
export { VERSION, Button, ButtonV2, Carousel, AIRich, Toolkit, sendHtmlView };
