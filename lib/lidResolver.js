...
async function resolveSenderJid(rawJid, chatJid, client, msgKey = null) {
    ...
    // Native fast path: Baileys already pairs PN/LID on the message key itself
    // (participantAlt / remoteJidAlt) whenever it has resolved that pairing.
    // Trust this first — it's a direct read from the protocol data, not a
    // reconstruction, so it's more reliable than cache/file/metadata lookups.
    if (msgKey) {
